 ...
 * Then opens "Share with someone…" and checks its title. Screenshot for the
 * record; the project is deleted at the end.
 */
  ...
  try {
    ...
    const dialog = await session.eval(`(() => {
      ...
      return { text: t.slice(0, 2000), hasRecipient: /recipient/i.test(t), sharedWith: /Shared with/.test(t),
        unsaved: /Unsaved changes \\(0\\)/.test(t), find: /Find people, teams or groups/.test(t),
    ...
    checks.tabSaysUnsavedChanges = Boolean(dialog && dialog.unsaved);
