"""Connection + project administration (RADD-1253).

Separate router from the webhook receiver on purpose: the receiver is
unauthenticated and verified by the hook token, everything here is admin-level
and verified by the permission engine.
"""
...
async def update_connection(
    ...
    return await _read(
        session, await service.update_connection(session, connection_id, data, actor_id=user.id)
    )
...
async def test_connection(
    ...
    """With a token: `GET /version`, reporting the GitLab version. Without: a
    one-row public project listing, proving the API host answers. Reports what
    went wrong rather than raising — a failure IS the answer."""
    await authz.require(session, user, authz.Permission.GLOBAL_MANAGE)
    connection = await service.get_connection(session, connection_id)
    path = "/version" if connection.api_token else "/projects?per_page=1"
    try:
        async with httpx.AsyncClient(
            verify=connection.verify_ssl, timeout=settings.gitlab_http_timeout_seconds
        ) as client:
        ...
        if connection.api_token:
            body = response.json()
            return ConnectionTest(ok=True, version=f"GitLab {body.get('version', '?')}")
        return ConnectionTest(ok=True, version=connection.api_url)
...
async def backfill_repo(
    ...
    """Walk the project's branches, merge requests (and the time logged on
    them) and commits, linking what the webhook never saw. Idempotent — every
    write goes through an upsert keyed by the host's own ids."""
    await authz.require(session, user, authz.Permission.VCSCONN_UPDATE)
    ...
    if not connection.api_token:
        raise HTTPException(
            status_code=422,
            detail="this connection has no API token; backfill reads the GitLab API",
        )
