...
async def list_options(session: AsyncSession, actor: User, *, q: str = "", limit: int = 50,
    ...
    readable = {identifier: permissions for identifier, permissions in readable.items()
                if authz.Permission.FORM_MANAGE in permissions}
    query = select(cast(Form.id, String).label("value"), Form.name.label("label"),
                   Project.key.label("hint")).join(Project, Project.id == Form.project_id)
    query = query.where(Form.project_id.in_(readable))
